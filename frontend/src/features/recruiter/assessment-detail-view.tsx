...
import {
  useRecruiterAssessment,
  useUpdateRecruiterAssessment,
} from '@/hooks/use-recruiter-assessment';
import {
  useRecruiterInvitations,
  useCreateRecruiterInvitation,
  useCancelRecruiterInvitation,
} from '@/hooks/use-recruiter-invitations';
...
} from './assessment-detail/components/candidates-tab';
import {
  CandidateInviteDialog,
  type CandidateInviteFormValues,
} from './assessment-detail/components/candidate-invite-dialog';
import {
  settingsFormSchema,
...
export function RecruiterAssessmentDetailView({
  basePath = '/recruiter',
}: {
  basePath?: string;
}) {
  ...
  const { data, isLoading, isError, error, refetch, isFetching } =
    useRecruiterAssessment(assessmentId);
  ...
  const settingsFormValues = useMemo(
    () => mapNormalizedToFormValues(normalizedSettings),
    [normalizedSettings]
  );
  ...
  const handleSettingsSubmit = async (e: React.FormEvent) => {
    ...
    await settingsForm.handleSubmit(async (values) => {
      ...
      const baselineSettings =
        createSettingsPayloadFromNormalized(normalizedSettings);

      console.log('Payload settings:', payloadSettings);
      console.log('Baseline settings:', baselineSettings);
      console.log(
        'Are equal?',
        settingsPayloadsEqual(payloadSettings, baselineSettings)
      );
      ...
      try {
        ...
        const message =
          mutationError instanceof Error
            ...
            : 'Unable to update assessment settings';
        showToast({
          title: 'Update failed',
          description: message,
          variant: 'destructive',
  ...
  const handleInvitationStatusChange = useCallback(
    (value: InvitationStatusFilter) => {
  ...
  const questionPool = useMemo(() => {
    ...
    return (questionPoolData?.items ?? []).filter(
      (item) => !existingIds.has(item.id)
    );
  }, [questionDraft, questionPoolData?.items]);

  const isQuestionPoolLoadingState =
    isQuestionPoolLoading || (questionDialogOpen && isQuestionPoolFetching);
  ...
  const isInvitationsLoadingState =
  ...
  const completionTrendData = useMemo(
    () => buildCompletionTrend(results),
    [results]
  );
  const invitationStatusSlices = useMemo(
    () => buildInvitationBreakdown(invitations),
    [invitations]
  );
  const proctoringSlices = useMemo(
    () => buildProctoringBreakdown(results),
    [results]
  );
  ...
  const resendInvitationMutation = useMutation({
    ...
    onSuccess: (response) => {
      showToast({
        ...
        description:
          response?.message ?? 'Candidate will receive a fresh invite shortly.',
        variant: 'success',
      });
      refetchInvitations();
    },
    ...
    () => [
      {
        ...
        cell: (row) => (
          <Checkbox
            ...
            onCheckedChange={(checked) =>
              handleToggleQuestionSelection(row.id, checked === true)
            }
          />
        ),
        className: 'w-[48px]',
  ...
  const handlePointsChange = useCallback(
    (questionId: string, nextPoints: number) => {
      setQuestionDraft((prev) =>
        prev.map((item) =>
  ...
  const handleSaveQuestions = useCallback(async () => {
    ...
    try {
      ...
      showToast({
        ...
        description:
          'Assessment flow now reflects the latest question order and scoring.',
        variant: 'success',
      });
    } catch (mutationError) {
      const message =
    ...
    async (values: MetadataFormValues) => {
      ...
      try {
        ...
        const message =
          mutationError instanceof Error
            ? mutationError.message
            : 'Unable to update assessment details';
        showToast({
          title: 'Update failed',
          description: message,
          variant: 'destructive',
  ...
  const handleDuplicate = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: recruiterKeys.assessments(),
        exact: false,
      });
    ...
    async (invitationId: string) => {
      ...
      try {
        ...
        showToast({
          ...
          description:
            'The candidate will no longer be able to access this assessment.',
          variant: 'default',
        });
        refetchInvitations();
      } catch (mutationError) {
  ...
  if (!assessmentId) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>
          We couldn&apos;t determine which assessment to load. Return to the
          assessments catalog and try again.
        </AlertDescription>
      </Alert>
    );
  }
          ...
          <Card>
            <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <CardDescription>
                  Reorder, re-score, and curate the assessment flow.
                </CardDescription>
              </div>
                ...
                >
                  ...
                  <DialogContent className="max-w-4xl">
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Select questions to append to this assessment.
                      </DialogDescription>
                    </DialogHeader>
                      ...
                      <DataTable<QuestionRecord>
                        ...
                        emptyMessage={
                          questionSearch.trim()
                            ...
                            : 'All available questions are already included.'
                        }
                        rowKey={(row) => row.id}
                        rowClassName={(row) =>
            ...
            <CardContent className="space-y-4">
              ...
              {questionDraft.length === 0 ? (
                <div className="flex items-center justify-center rounded-lg border border-dashed border-border/60 py-12 text-sm text-muted-foreground">
                  No questions linked yet. Add questions from your bank to build
                  the assessment.
                </div>
              ) : (
                          ...
                          <TableCell>
                            <div className="flex justify-end gap-1">
                              <Button
                                ...
                                className="h-8 w-8"
                                onClick={() =>
                                  handleMoveQuestion(item.questionId, 'up')
                                }
                              ...
                              <Button
                                ...
                                className="h-8 w-8"
                                onClick={() =>
                                  handleMoveQuestion(item.questionId, 'down')
                                }
                                disabled={
                                  index === questionDraft.length - 1 ||
                                  isQuestionsSubmitting
                                }
                              >
            ...
            >
              <Card>
                <CardHeader>
                  <CardTitle>Assessment settings</CardTitle>
                  <CardDescription>
                    Adjust timing, attempts, review options, and proctoring
                    signals.
                  </CardDescription>
                </CardHeader>
                  ...
                  <div className="space-y-4 rounded-lg border border-border/60 bg-muted/30 p-4">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Decide which monitoring signals you need for this
                        assessment.
                      </p>
                    </div>
                    <div className="grid gap-3 md:grid-cols-2">
            ...
            <CardHeader className="space-y-3">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <CardDescription>
                    Review performance, risk signals, and export-ready data.
                  </CardDescription>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <Input
              ...
              {isResultsLoadingState ? (
                ...
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {results.map((result) => {
                        const percent = formatScorePercentage(
                          result.score,
                          result.total
                        );
                        return (
        ...
        <TabsContent value="analytics" className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-3">
            <AnalyticsMetric
              ...
              helper="Across completed submissions"
            />
            <AnalyticsMetric
              title="Highest score"
              value={`${scoreStats.highest}%`}
              helper="Best candidate performance"
            />
            <AnalyticsMetric
              title="Lowest score"
              value={`${scoreStats.lowest}%`}
              helper="Lowest observed score"
            />
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Average score across submissions over time.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {scoreTrend.length === 0 ? (
                  ...
                  <ChartContainer
                    config={{
                      score: { label: 'Average score', color: CHART_COLORS[0] },
                    }}
                    className="h-[240px]"
                  >
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Daily completions for this assessment.
                </CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Where candidates sit in the journey.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {invitationStatusSlices.length === 0 ? (
                  ...
                  <ChartContainer
                    config={invitationStatusSlices.reduce(
                      (acc, slice, index) => {
                        acc[slice.key] = {
                          label: slice.label,
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Signals observed across submissions.
                </CardDescription>
              </CardHeader>
              <CardContent>
