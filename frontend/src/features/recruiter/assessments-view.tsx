...
export function RecruiterAssessmentsView({
  ...
  const [archiveContext, setArchiveContext] =
    useState<ArchiveActionContext | null>(null);
  ...
  const recordRecentStatus = useCallback(
    (ids: string[], status: AssessmentSummary['status']) => {
  ...
  const averageDuration = assessments.length
    ? (
        assessments.reduce(
          (total, current) => total + (current.timeLimit ?? 0),
          0
        ) / assessments.length
      ).toFixed(1)
    : '0.0';
  ...
  useEffect(() => {
    setSelectedIds((prev) =>
      prev.filter(
        (id) =>
          selectedMap[id] !== undefined || assessments.some((a) => a.id === id)
      )
    );
    ...
    onError: (error, variables) => {
      const message =
        error instanceof Error
          ? error.message
          : 'Unable to publish selected assessments';
  ...
  const unpublishMutation = useMutation({
    ...
    onError: (error, variables) => {
      const message =
        error instanceof Error
          ? error.message
          : 'Unable to update selected assessments';
      if (variables.source === 'bulk') {
        setBulkError(message);
      }
      showToast({
  ...
  const archiveMutation = useMutation({
    ...
    onError: (error, variables) => {
      const message =
        error instanceof Error
          ? error.message
          : 'Unable to archive selected assessments';
      if (variables.source === 'bulk') {
        setBulkError(message);
      }
      showToast({
  ...
  const rangeLabel = useMemo(() => {
    ...
    return `${Math.min(start, pagination.total)}–${Math.max(
      start,
      Math.min(end, pagination.total)
    )}`;
        ...
        cell: (row) => (
          <Checkbox
            ...
            onCheckedChange={(checked) =>
              toggleSelectOne(row.id, checked === true)
            }
          />
        ),
        className: 'w-[48px]',
        ...
        cell: (row) => {
          ...
          return (
            <div className="flex flex-col items-start gap-1">
              ...
              {message ? (
                <span className={`text-xs font-medium ${messageTone}`}>
                  {message}
                </span>
              ) : null}
            </div>
    ...
    <div className="grid gap-6">
        ...
        <Alert variant="destructive">
          ...
          <AlertDescription>
            {(error as Error | undefined)?.message ??
              'Double-check that the recruiter assessments API is available.'}
          </AlertDescription>
        </Alert>
      ) : null}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Snapshot of live, draft, and archived assessments.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <SummaryMetric
            label="Archived"
            value={`${archivedCount}`}
            helper="Parked or inactive"
          />
          <SummaryMetric
            label="Avg. duration"
            value={`${averageDuration} min`}
            helper="Across listed assessments"
          />
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="gap-4">
          <div className="flex flex-col gap-2">
            ...
            <CardDescription>
              Search, filter, and paginate assessments served directly from the
              recruiter API.
            </CardDescription>
          </div>
        ...
        <CardContent className="space-y-4">
                ...
                <AlertDialogTitle>
                  {archiveContext
                    ? archiveContext.type === 'single'
                      ...
                      : `Archive ${
                          archiveContext.assessments.length
                        } assessment${
                          archiveContext.assessments.length === 1 ? '' : 's'
                        }?`
                    : 'Archive assessments'}
                </AlertDialogTitle>
                <AlertDialogDescription>
          ...
          <DataTable
            ...
            rowKey={(row) => row.id}
            rowClassName={(row) =>
              selectedIds.includes(row.id) ? 'bg-primary/5' : undefined
            }
          />
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-muted-foreground">
              Showing {rangeLabel} of {pagination.total} assessment
      ...
      <Card className="border-dashed">
        <CardHeader>
          ...
          <CardDescription>
            Create from scratch or reuse questions from your bank.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Button asChild>
